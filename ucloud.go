// Copyright 2023 The Alone88 Authors. All Rights Reserved.
//
...
func GetUcloudClient(publicKey string, privateKey string, projectId string, sign string, template string) (*UcloudClient, error) {
	if len(publicKey) == 0 {
		return nil, fmt.Errorf("missing parameter: publicKey")
	}
	if len(privateKey) == 0 {
		return nil, fmt.Errorf("missing parameter: privateKey")
	}
